...
enum conn_state { CS_INIT, CS_DONE };
...
void launch_server(struct param *param) {
  ...
  for (int i = 0; i < MAX_CONN; ++i) {
    ...
    m.conns[i]->rx_mem = m.conns[i]->tx_mem + param->size * TX_DEPTH;
...
static int client_send(rdma_t *rdma, struct conn_ctx *c, size_t size) {
  ...
  for (int i = 0; i < c->count; ++i) {
    ...
    if (c->cq_mod == 0)
      ...
      notify = true;
    rc = rdma->send(c->con, &c->tx_ctx[i], 1, &c->tx_ctx[i], notify);
    if (rc) {
...
static int poll_send(rdma_t *rdma, struct conn_ctx *c, struct thrd_msg *m) {
  ...
  for (int i = 0; i < n; ++i) {
    if (wc[i].status != IBV_WC_SUCCESS) {
      debug("send wc[%d] %s", i, ibv_wc_status_str(wc[i].status));
      return -1;
    }
...
static int poll_recv(rdma_t *rdma, struct conn_ctx *c, struct thrd_msg *m) {
  ...
  for (int i = 0; i < n; ++i) {
    if (wc[i].status != IBV_WC_SUCCESS) {
      debug("recv wc[%d] %s", i, ibv_wc_status_str(wc[i].status));
      return -1;
    }
    m->io += 1;
    m->rx_sz += m->size;
...
static int client_poll(rdma_t *rdma, struct conn_ctx *head,
                       struct thrd_msg *msg) {
...
static struct conn_ctx *remove_node(struct conn_ctx *head,
                                    struct conn_ctx **n) {
...
static void *client_func(void *arg) {
  ...
  while (head) {
    if (g_verbose)
      debug("conn %p ldev %d rdev %d", head, head->con->dev, head->con->rdev);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "s:d:c:n:p:i:SCVh")) != -1) {
    ...
    case 's':
      ...
      if (param.size < PAGE_SZ || param.size > MAX_SZ) {
        debug("size out of range, expect [%lu, %lu]", PAGE_SZ, MAX_SZ);
        return 1;
      }
      break;
    case 'S':
  ...
  printf("]\n");
  printf("%10s\t%d\n%10s\t%d\n", "conns", param.conns, "port", param.port);
  if (param.ip[0])
    printf("%10s\t%s\n", "ip[0]", param.ip[0]);
  if (param.ip[1])
    printf("%10s\t%s\n", "ip[1]", param.ip[1]);
  if (!param.ip[0])
    printf("%10s\t%s\n", "use_srq", param.use_srq ? "yes" : "no");
  printf("%10s\t%d\n", "IB", param.card);
  printf("%10s\t%d\n", "cq_mod", param.cq_mod);
